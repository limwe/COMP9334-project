    ...
    if t in range(trace_mode_test_start, trace_mode_test_end + 1):
        ...
        if test_mrt:
            ...
            try:
                ...
                if np.allclose(mrt_stu, mrt_ref, atol=ABS_TOL):
                    print(
                        "Test " + str(t) + ": Mean response time matches the reference"
                    )
                else:
                    print(
                        "Test "
                        + str(t)
                        + ": Mean response time does NOT match the reference"
                    )
            except:
                print("Something went wrong with the comparison.")
                print(
                    f"The contents of the mrt_{t}.txt that your program has produced are:"
                )
        ...
        if test_dep:
            ...
            try:
                ...
                if (
                    dep_stu_times.shape == dep_ref_times.shape
                    and dep_stu_class.shape == dep_ref_class.shape
                ):
                    if np.allclose(
                        dep_stu_times, dep_ref_times, atol=ABS_TOL
                    ) and np.all(dep_stu_class == dep_ref_class):
                        print(
                            "Test " + str(t) + ": Departure times match the reference"
                        )
                    else:
                        print(
                            "Test "
                            + str(t)
                            + ": Departure times do NOT match the reference"
                        )
                else:
                    ...
                    print(
                        f"Error: Your dep_{t}.txt has {nr_stu} rows and {nc_stu} columns but we expect {nr_ref} rows and {nc_ref}"
                    )
            except:
                print("Something went wrong with the comparison.")
                print(
                    f"The contents of the dep_{t}.txt that your program has produced are:"
                )
        ...
        for idx in [0, 1]:
            if (
                MRT_TOL[t][classes[idx]][0]
                <= mrt_stu[idx]
                <= MRT_TOL[t][classes[idx]][1]
            ):
                print("Test " + str(t) + ": Mean response time is within tolerance")
            else:
                print("Test " + str(t) + ": Mean response time is NOT within tolerance")
                print(
                    "You should try to run a new simulation round with new random numbers."
                )
                print(
                    "Your output need to be within the tolerance for most of the rounds."
                )
